...
class AdminTermController extends AdminbaseController {
	...
	public function add_post(){
		if (IS_POST) {
         $_POST['smeta']['thumb'] = sp_asset_relative_url($_POST['smeta']['thumb']);
            ...
            $terms=I("post.post");
            $terms['smeta']=json_encode($_POST['smeta']);
            $terms['comment']=htmlspecialchars_decode($terms['comment']);
			if ($this->terms_model->create()!==false) {
                $this->terms_model->smeta=$terms['smeta'];
	...
	public function edit(){
		...
		$tree_tpl="<option value='\$id' \$selected>\$spacer\$name</option>";
		$tree=$tree->get_tree(0,$tree_tpl);
        $this->assign("smeta",json_decode($terms['smeta'],true));
		$this->assign("terms_tree",$tree);
